...
A companion notebook (`/python/LendingClub_Python_Companion.ipynb`) reproduces every headline figure on this page independently in pandas/NumPy/matplotlib, working from the raw CSV. Every number reproduces within rounding; the Estimated Excess Loss figure matches to the cent. Rebuilding the pipeline a second time also surfaced two things worth mentioning (both folded into the notes below): the true shape of the "33 blank rows" cleaning step, and an implicit modeling assumption in the Estimated Excess Loss calculation. See `/python/README.md` for how to run it.

## Limitations and honest notes

- **The "33 blank rows" aren't blank.** Re-doing the source-row cleaning step in pandas (rather than trusting the original "id not null" description) showed they're leftover footer/subtotal rows the CSV export embedded directly in the data ("Total amount funded in policy code 1: 6417608175"), where `id` holds a text label, not a null. Filtering on a genuine numeric column being non-null catches them reliably; filtering on `id` only works once it's converted to a numeric type, so the text label errors out to null on conversion — a detail the original Power Query step relied on. Same 33 rows removed either way; this is a more precise account of why.
